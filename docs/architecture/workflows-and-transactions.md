...
| `commit_import` | Acquisition, Catalog, Reading, Codex | An import commit (create/append novel, create source, upsert chapters, register assets, invalidate codex ranges, finalize the import job) runs as one prepared operation over an `ImportCommitApis` bundle of the four owners. Automatic volume appends acquire an Acquisition-owned, transaction-scoped advisory lock keyed by target novel before allocating the next chapter range; single- and batch-volume appends share the same lock namespace. |
...
| `commit_codex_extraction` | Reading, Codex | Re-reads the chapter **with a row lock inside the transaction**, recomputes its SHA-256, then Codex takes its per-novel advisory lock and freshly rebuilds the bounded context hash. `source_changed` or `stale_extraction_context` aborts before atomically committing historical claims, temporal state, activity, threads, chapter/hierarchical summaries, context manifest, and extraction checkpoint. |
