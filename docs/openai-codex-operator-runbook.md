...
4. Copy the OpenAI Codex settings from `.env.example`. Set the binary hash to the hash from
   step 2. Keep both enable switches false initially.
...
output schema. Story data is passed as explicitly untrusted task data. The host—not the
model—writes the output files and SHA-256 manifest, after which the existing translation or
extraction validators and atomic commit workflows run.
...
- Authentication/version/model/protocol drift makes the worker unhealthy and prevents
  claims. Re-run preflight before re-enabling.
