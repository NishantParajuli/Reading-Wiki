...
integration. The agent proposes artifacts under `output/`; a trusted stop hook finalizes
the complete manifest and hashes, then NovelWiki validates and commits them. Workload
instructions are inlined into the initial print prompt because AGY 1.1.2 can loop while
activating a discovered workspace skill.

Entity-disambiguation instructions inline the complete decision shape. The stop hook checks
