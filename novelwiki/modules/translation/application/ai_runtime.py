"""Translation-owned ports for provider, AGY, and Work capabilities."""
...
def _ai_runtime():
    if _ai is None:
        raise RuntimeError("Translation AI capabilities were not wired by the composition root")
    return _ai
...
class _WorkProxy:
    def __getattr__(self, name):
        if _work is None:
            raise RuntimeError("Translation Work capabilities were not wired by the composition root")
...
def _forward(name):
    def call(*args, **kwargs):
        return getattr(_ai_runtime(), name)(*args, **kwargs)
    return call
